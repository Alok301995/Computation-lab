...
HeapNode* extractMinIndex(Heap *heap){
    if(isEmpty(heap)){ // no element in the heap
        return NULL;
    }
    HeapNode *min = (HeapNode *)malloc(sizeof(HeapNode));
    int minIndex = heap ->arr[0] ->vertex;
    min ->vertex = minIndex;
    min ->time = heap ->arr[0] ->time;
...
void printHeap(Heap *heap){
    for( int i=0; i<heap ->size; i++){
    printf("Heap index %d --> Heap weight %d --> time %f\n" , heap ->arr[i] ->vertex ,heap->arr[i] ->weight , heap  ->arr[i] ->time);
...
void kShortestDistanceSource(Node **ptr ,int *dist , int *visited , int source , int k , Heap *heap , int size){
    int sourceTime = heap ->arr[source] ->time;
    ...
    for(int i=0 ; i<size ;i++){
        HeapNode *min = extractMinIndex(heap);
        minIndex =  min ->vertex;
        time = min ->time;
        if(counter == k){
            return;
        }
        if(minIndex != source && time > -1){
            if(time < sourceTime ){
                counter++;
                printf("%d \n", minIndex +1);
            }
...
int main(){
    ...
    fclose(file);
    kShortestDistanceSource(nodeptr ,dist , visited ,S-1,K ,heap ,N);
    return 0;
}